...
See \ref{subsec:nlp_tunbert_implementation} for details on NLP-specific preprocessing.%todo verify ref later

\subsection{Speech Emotion Recognition (SER)}
\label{subsec:ser_implementation}
The SER module is implemented within the \texttt{audio\_features} function. It first utilizes the \textbf{Librosa} library to load the input audio file and resample it to the required 16kHz. The core 
of the module is the pre-trained \textbf{\texttt{speechbrain/emotion-recognition-wav2vec2-IEMOCAP}} model, which is loaded using the \texttt{EncoderClassifier} class from the SpeechBrain library. 
...
The \texttt{transcribe\_audio} function handles the ASR task. It instantiates a pipeline object from the Hugging Face \textbf{\texttt{transformers}} library, specifically configured for \texttt{"automatic-speech-recognition"}. This pipeline leverages OpenAI's \textbf{Whisper} model (the \texttt{base} version) to perform robust, zero-shot transcription on the audio file, converting the user's speech into a text string for the NLP module.

\subsection{Natural Language Processing (TunBERT Focus)}
\label{subsec:nlp_tunbert_implementation}

\textbf{Model Selection and Fine-tuning:} The NLP module uses a fine-tuned version of \textbf{TunBERT}, a BERT variant for Tunisian Arabic. The fine-tuning process, documented in the \texttt{fineTuneTunBERT.ipynb} notebook, was implemented using the \texttt{AutoTokenizer} and \texttt{AutoModelForSequenceClassification} classes from the \texttt{transformers} library. The model was trained on a custom-built, labeled dataset of Tunisian distress-related phrases.

\textbf{Real-Time Inference:} For live prediction, the \texttt{classify\_text} function loads the fine-tuned TunBERT model and its tokenizer. It takes the text string from the ASR module as input, tokenizes it, and passes the resulting tensors to the model to get the final intent classification (e.g., "Distress," "Normal").
\textbf{Model Selection and Architecture}: TunBERT, a BERT variant for Tunisian Arabic, is selected for intent analysis.
\label{subsubsec:tunbert_finetuning}
\textbf{Fine-tuning and Dataset Preparation}: Fine-tuned on a custom Tunisian distress dataset, preprocessed for context.
\label{subsubsec:tunbert_training}
\textbf{Training Configuration}: Optimized with low learning rates and batch processing for efficiency.
...
\subsubsection{Natural Language Processing (TunBERT Focus)}
\label{subsec:nlp_implementation}

The NLP module is responsible for the final stage of analysis: determining user intent from the transcribed text. This was accomplished by fine-tuning and implementing a specialized Transformer model.

\paragraph{Model Selection and Architecture}
For this task, \textbf{TunBERT} was selected, which is a BERT-based model specifically pre-trained on a large corpus of Tunisian Arabic. This provides a strong foundation for understanding the nuances, slang, and code-switching present in the local dialect.

\paragraph{Fine-tuning and Dataset Preparation}
The core of the implementation was fine-tuning the base TunBERT model for the specific task of distress detection. This process, documented in the \texttt{fineTuneTunBERT.ipynb} notebook, utilized the \texttt{AutoTokenizer} and \texttt{AutoModelForSequenceClassification} classes from the Hugging Face \texttt{transformers} library. A custom dataset of Tunisian dialect phrases related to distress scenarios was collected, manually labeled, and cleaned. This dataset was then tokenized and split into training and validation sets.

\paragraph{Training Configuration and Inference}
...
%todo pipeline image should be present
...
Schuller, B., "Speech Emotion Recognition: Two Decades in a Nutshell," Frontiers in Digital Humanities, 2018.
\end{thebibliography}
