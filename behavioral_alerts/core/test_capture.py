...
# Fake collections to replace MongoDB
class FakeLocations:
...
class FakeUsers:
    def find_one(self, query):
        return {
            "user_id": "user123",
            "profile": {"centroids": [], "hour_freq": {}, "weekday_freq": {}, "month_freq": {}},
            "threshold_value": 0.5,
        }

# Fake anomaly detection function
def fake_detect_user_anomalies(user_id, lat, lon):
    return (0.8, 0.7, 0.6, 0.9)  # high anomalies for test

def fake_detect_user_anomalies_low(user_id, lat, lon):
    return (0.1, 0.1, 0.1, 0.1)  # low anomalies for test

# Fake log_alert
def fake_log_alert(**kwargs):
    fake_log_alert.called = True
    fake_log_alert.kwargs = kwargs
    return "alert-uuid"
fake_log_alert.called = False
fake_log_alert.kwargs = {}

# Tests
def test_check_anomalies_for_user_anomaly(monkeypatch):
    import behavioral_alerts.core.anomaly_checker as anomaly_checker

    # Patch everything
    monkeypatch.setattr(anomaly_checker, "locations_collection", FakeLocations())
    monkeypatch.setattr(anomaly_checker, "users_collection", FakeUsers())
    monkeypatch.setattr(anomaly_checker, "detect_user_anomalies", fake_detect_user_anomalies)
    monkeypatch.setattr(anomaly_checker, "log_alert", fake_log_alert)

    # Set threshold low so anomaly triggers incident
    FakeUsers().find_one = lambda query: {
        "user_id": "user123",
        "profile": {"centroids": [], "hour_freq": {}, "weekday_freq": {}, "month_freq": {}},
        "threshold_value": 0.5
    }

    result = anomaly_checker.check_anomalies_for_user("user123")

    assert result is True
    assert fake_log_alert.called is True
    assert fake_log_alert.kwargs["incident_probability"] > 0.5

def test_check_anomalies_for_user_no_anomaly(monkeypatch):
    import behavioral_alerts.core.anomaly_checker as anomaly_checker

    # Patch everything
    monkeypatch.setattr(anomaly_checker, "locations_collection", FakeLocations())
    monkeypatch.setattr(anomaly_checker, "users_collection", FakeUsers())
    monkeypatch.setattr(anomaly_checker, "detect_user_anomalies", fake_detect_user_anomalies_low)
    monkeypatch.setattr(anomaly_checker, "log_alert", lambda **kwargs: pytest.fail("log_alert should not be called"))

    # Set threshold high so anomaly does NOT trigger incident
    FakeUsers().find_one = lambda query: {
        "user_id": "user123",
        "profile": {"centroids": [], "hour_freq": {}, "weekday_freq": {}, "month_freq": {}},
        "threshold_value": 0.9
